...
# Assuming your dataframes are named cc.db and ref
...
common_genes_test <- intersect("ACE2", ref$SYM)
...
# list related biological process
...
# norm.count is a dataframe
# Extract row names
row_names_coutData <- rownames(countData)

# Create a new dataframe with rows that have NOX5 after the "=" sign
nox5_df_countData <- countData[grepl("=NOX5$", row_names_coutData), ]

# Display the new dataframe
print(nox5_df_countData)

# Finalized for GitHub release (2025.04.03). Note: NOX5 gene with two Ensembl IDs – issue currently unresolved.